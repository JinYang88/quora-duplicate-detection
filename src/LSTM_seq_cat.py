...
TEXT = data.Field(sequential=True, lower=True, eos_token='<EOS>', init_token='<BOS>',
                  pad_token='<PAD>', batch_first=True, use_vocab=True)
...
MODEL = LSTM_seq_cat(len(TEXT.vocab), embedding_dim, hidden_dim, batch_size, word_matrix, bidirectional)
if device == 0:
    MODEL.cuda()
        ...
        for text1, text2, label in train_dl:
            ...
            if batch_count % print_every == 0:
                ...
                if acc > max_metric:
                    ...
                    max_metric = acc
                    print("Saving model..")
                    torch.save(best_state, '../model_save/LSTM_seq_cat.pth')           
...
loss, (acc, Precision, Recall, F1) = predict_on(MODEL, test_dl, nn.NLLLoss(), device, '../model_save/LSTM_seq_cat.pth')
